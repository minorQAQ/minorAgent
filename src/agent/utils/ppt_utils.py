...
支持:
  ...
  - pptx → HTML: 将 PPTX 转为带翻页导航的 HTML 预览页
...
# ============================================================
# 14. PPTX → HTML 预览（python-pptx）
# ============================================================

def convert_pptx_to_html(pptx_path: Path) -> str:
    """将 PPTX 文件转为带翻页导航的完整 HTML 页面。

    使用 python-pptx 解析，提取文本/图片/形状/表格，
    生成绝对定位 HTML + 翻页控件 + 键盘/滚轮导航 + 拖动/缩放。
    """
    ...
    def _emu2px(emu):
        if emu is None:
            return 0
        return round(float(emu) / 914400 * 96, 1)

    def _fill_style(shape):
        try:
            f = shape.fill
            if f.type is not None and f.type != 0:
                if hasattr(f, 'fore_color') and f.fore_color.type is not None:
                    try:
                        return f"background:{f.fore_color.rgb};"
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _line_style(shape):
        try:
            ln = shape.line
            if ln.fill.type is not None and ln.fill.type != 0:
                w = _emu2px(ln.width) if ln.width else 0.5
                c = "#999"
                try:
                    if hasattr(ln, 'color') and ln.color.type is not None:
                        c = str(ln.color.rgb)
                except Exception:
                    pass
                return f"border:{w}px solid {c};"
        except Exception:
            pass
        return ""
    ...
    def _run_style(run):
        ...
        f = run.font
        if f.size:
            s.append(f"font-size:{_emu2px(f.size)}px")
        if f.bold:
        ...
        try:
            if f.color and f.color.type is not None:
                s.append(f"color:{f.color.rgb}")
        except Exception:
            pass
        if f.name:
            s.append(f'font-family:"{f.name}",sans-serif')
        return ";".join(s)
    ...
    def _render_text(tf, l, t, w, h):
        ...
        for para in tf.paragraphs:
            ...
            for run in para.runs:
                st = _run_style(run)
                txt = _esc(run.text).replace("\n", "<br>")
                runs.append(f'<span style="{st}">{txt}</span>' if st else txt)
            line = "".join(runs) or "&nbsp;"
            m = ""
            if para.space_after:
                m += f"margin-bottom:{_emu2px(para.space_after)}px;"
            if para.space_before:
                m += f"margin-top:{_emu2px(para.space_before)}px;"
            parts.append(f'<p style="{al}{m}">{line}</p>')
        inner = "".join(parts)
        pad = ""
        try:
            pad = f"padding-left:{_emu2px(tf.margin_left or 0)}px;padding-right:{_emu2px(tf.margin_right or 0)}px;padding-top:{_emu2px(tf.margin_top or 0)}px;"
        except Exception:
            pass
        return (
            f'<div style="position:absolute;left:{l}px;top:{t}px;width:{w}px;height:{h}px;'
            f'overflow:hidden;word-wrap:break-word;{pad}">{inner}</div>'
        )

    def _render_pic(shape):
        l, t, w, h = _emu2px(shape.left), _emu2px(shape.top), _emu2px(shape.width), _emu2px(shape.height)
        try:
            img = shape.image
            ...
            src = ""
        return f'<img src="{src}" style="position:absolute;left:{l}px;top:{t}px;width:{w}px;height:{h}px;object-fit:contain;" />'

    def _render_shape(shape):
        l, t, w, h = _emu2px(shape.left), _emu2px(shape.top), _emu2px(shape.width), _emu2px(shape.height)
        fill = _fill_style(shape)
        line = _line_style(shape)
        rad = ""
        try:
            if shape.auto_shape_type and "ROUNDED" in str(shape.auto_shape_type):
                rad = "border-radius:6px;"
        except Exception:
            pass
        if shape.has_text_frame:
            inner = _render_text(shape.text_frame, 0, 0, w, h)
            return (
                f'<div style="position:absolute;left:{l}px;top:{t}px;width:{w}px;height:{h}px;'
                f'{fill}{line}{rad}overflow:hidden;">{inner.replace("position:absolute;", "position:relative;")}</div>'
            )
        return f'<div style="position:absolute;left:{l}px;top:{t}px;width:{w}px;height:{h}px;{fill}{line}{rad}"></div>'

    def _render_table(shape):
        l, t, w = _emu2px(shape.left), _emu2px(shape.top), _emu2px(shape.width)
        rows = []
        for row in shape.table.rows:
            cells = []
            for cell in row.cells:
                txt = _esc(cell.text) or "&nbsp;"
                cells.append(f'<td style="border:1px solid #555;padding:3px 6px;font-size:11px;vertical-align:top;color:#ddd;">{txt}</td>')
            rows.append(f'<tr>{"".join(cells)}</tr>')
        return f'<table style="position:absolute;left:{l}px;top:{t}px;width:{w}px;border-collapse:collapse;background:#2a2a2a;font-family:sans-serif;">{"".join(rows)}</table>'

    def _render_group(shape):
        l, t, w, h = _emu2px(shape.left), _emu2px(shape.top), _emu2px(shape.width), _emu2px(shape.height)
        kids = []
        try:
            for child in shape.shapes:
                kids.append(_el(child))
        except Exception:
            pass
        return f'<div style="position:absolute;left:{l}px;top:{t}px;width:{w}px;height:{h}px;overflow:visible;">{"".join(kids)}</div>'
    ...
    def _el(shape):
        ...
        if "PICTURE" in st:
            return _render_pic(shape)
        elif "TABLE" in st:
            return _render_table(shape)
        elif "GROUP" in st:
            return _render_group(shape)
        elif "PLACEHOLDER" in st or "TEXT_BOX" in st:
            if shape.has_text_frame:
                return _render_text(shape.text_frame, _emu2px(shape.left), _emu2px(shape.top), _emu2px(shape.width), _emu2px(shape.height))
            return ""
        elif "AUTO_SHAPE" in st or "FREEFORM" in st or "RECTANGLE" in st or "OVAL" in st:
    ...
    sw = _emu2px(prs.slide_width)
    sh = _emu2px(prs.slide_height)
    ...
    for slide in prs.slides:
        ...
        slides_data.append(
            ...
            f'background:#ffffff;overflow:hidden;flex-shrink:0;">'
            f'{"".join(elems)}</div>'
        )
...
body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#1a1a1a;height:100vh;overflow:hidden;user-select:none;position:relative}}
#vp{{position:absolute;inset:0;overflow:hidden}}
#wrap{{transform-origin:0 0;transition:none;will-change:transform}}
#wrap>div{{box-shadow:0 2px 20px rgba(0,0,0,.6)}}

</style></head>
<body>
<div id="vp"><div id="wrap"></div></div>
<script>
var slides={slides_json};
var total={total},cur=0;
...
function show(n){{
  ...
  cur=n;wrap.innerHTML=slides[cur];
  doFit();
}}
...
function doFit(){{
  ...
  scale=Math.min(vw/sw,vh/sh,1.0);
...
vp.addEventListener("wheel",function(e){{
  if(e.ctrlKey||e.metaKey){{
    ...
    var f=e.deltaY>0?.9:1.1,ns=Math.max(.2,Math.min(5,scale*f));
    ox=mx-(mx-ox)*(ns/scale);oy=my-(my-oy)*(ns/scale);
    scale=ns;apply();
    return;
  }}
  if(Math.abs(e.deltaY)>30){{show(cur+(e.deltaY>0?1:-1));e.preventDefault()}}
}},{{passive:false}});

vp.addEventListener("mousedown",function(e){{
  if(e.button===1||(e.button===0&&e.altKey)){{
    e.preventDefault();dragging=true;dsX=e.clientX;dsY=e.clientY;doX=ox;doY=oy;vp.style.cursor="grabbing";
  }}
}});
window.addEventListener("mousemove",function(e){{
  if(!dragging)return;ox=doX+(e.clientX-dsX);oy=doY+(e.clientY-dsY);apply();
}});
window.addEventListener("mouseup",function(){{if(dragging){{dragging=false;vp.style.cursor="";}}}});
...
document.addEventListener("keydown",function(e){{
  ...
  if(e.ctrlKey&&e.key==="0"){{doFit();e.preventDefault()}}
}});
window.addEventListener("resize",function(){{clearTimeout(fitT);fitT=setTimeout(doFit,100);}});
show(0);
</script>
</body></html>"""
