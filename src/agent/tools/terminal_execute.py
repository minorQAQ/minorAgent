...
class TerminalExecute(BaseTool):
    ...
    description: str = (
        ...
        "【数据作图标准流程】（本工具替代 data_plot，用于基于已入库 MySQL 数据作图）\n"
        "当用户要求对已入库的数据画图/出图表时，按以下流程用本工具执行一个 Python 脚本：\n"
        "1. 查库：脚本内用 pymysql 连接 MySQL。连接参数从 env_config.json 的\n"
        "   MYSQL_HOST/MYSQL_PORT/MYSQL_USER/MYSQL_PASSWORD 读取，或直接\n"
        "   `from agent.core.db import load_mysql_config` 获取；执行只读 SELECT 取数。\n"
