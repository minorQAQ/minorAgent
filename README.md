...
基于 **LangGraph** 构建，具备 GUI 自动化、浏览器控制、终端执行、RAG 知识库、邮件收发、图像生成、语音对话、数据库操作、定时任务等 19 项工具，并提供 Electron 桌面端一键安装体验。
...
- 🧩 **19 项内置工具**：从终端命令到邮件收发，从 PPT 制作到图像生成，覆盖真实办公场景
- 🏠 **完全本地运行**：Electron 打包，数据不出本机；后端 FastAPI + 前端零延迟直读磁盘
- 🎨 **可深度定制**：主题 / 字体 / 字号 / 背景 / 工作空间 / 提示词分层架构，全部可调
- 🔁 **工程级稳定性**：循环检测、上下文压缩、人工确认、轨迹回放，对抗 LLM 的"幻觉操作"
...
| 🔍 **web_search** | `tools/web_search.py` | 联网搜索（Tavily / Firecrawl） |
...
| 🤝 **human_interaction** | `tools/human_interaction.py` | 主动询问用户、收集信息 |
...
| 🗣️ **streaming_client** | `tts/streaming_client.py` | 流式 TTS 语音合成客户端 |
...
| **上下文膨胀** | 简单截断 | **图内分级压缩**：50% 窗口轻量压缩 / 75% 窗口全量压缩，每步 LLM 调用后检查 |
...
<br>
<sub><b>人机交互确认</b> — 敏感操作（终端执行、文件写入等）弹出确认浮窗，用户可同意、拒绝或修改指令&emsp;|&emsp;<b>任务规划 TodoList</b> — 多步任务自动拆解与进度跟踪，支持主 / 子 Agent 独立任务列表</sub>
</p>
...
│   内含: 反思注入 · 压缩 · 循环检测 · 路由 · 人工确认           │
└───────────────────────────────┬──────────────────────────────┘
                                 │ 调用
                                 ▼
┌──────────────────────────────────────────────────────────────┐
│             19 项工具 (tools/*.py) · 5 项技能 (skills/)        │
└──────────────────────────────────────────────────────────────┘
```
...
```
START → [agent: call_model] ──有 tool_calls──▶ [tools: ToolNode] ──▶ [process_tool_artifact]
              ▲                                  ◀────────────────────────┘
              │ 无 tool_calls
              ▼
             END
...
- **`call_model`**（`core/nodes.py`）：注入反思提示 → LLM 调用 → 每步检查压缩 → 提取 thinking
- **`should_continue`**（`core/routing.py`）：含 tool_calls → 检查确认策略 / 循环检测 → 路由到 `tools` 或 `END`
- **`process_tool_artifact`**：gui 截图作为 synthetic HumanMessage 注入，实现视觉闭环

### 多 Agent 模式

```
START → [supervisor] ──main完成──▶ [parallel_sub_agents] ──并行完成──▶ [supervisor]
                                  │                                     │
                                  └─ 子Agent结果 tag_message 注入主Agent ◄┘
                                              有 agent_call → 循环；无 → END
```
...
│   ├── agent/                   # 🧠 Agent 核心
│   │   ├── agents/
│   │   │   └── agent_runtime.py #   编排层入口：execute_agent / continue_after_human_action
│   │   ├── core/
...
│   │   │   ├── loop_detector.py #   循环检测：重复工具 / TodoList 停滞，两级响应
│   │   │   ├── llm.py           #   ChatQwen 多模态封装（图像/附件/非标准 tool_call）
│   │   │   ├── tool_policy.py   #   工具执行策略（直执 / 需确认）
│   │   │   ├── approvals.py     #   待确认操作注册与 UI 元数据
...
│   │   │   ├── storage.py        #     持久化层（CronTaskRepository，JSON / MySQL）
...
│   │   ├── history/
│   │   │   ├── session_storage.py     # 会话与消息持久化（JSON / MySQL 双后端）
│   │   │   └── tool_call_recorder.py  # 每轮工具调用轨迹记录 (turn_{id}.json)
...
| `WORKING_DIR` | `.` | 工作根目录（桌面端自动设为安装目录） |
| `WORKSPACE_DIR` | `workspace` | 默认工作空间子目录 |
| `USER_PYTHON_PATH` | `""` | 指定 Python 解释器路径 |
| `LLM_CONTEXT_WINDOW` | `262144` | 上下文窗口大小（token） |
| `MINI_COMPRESS_RATE` | `0.4` | 轻量压缩阈值（占窗口 40% 触发） |
| `HARD_COMPRESS_RATE` | `0.7` | 全量压缩阈值（占窗口 70% 触发） |
| `CURRENT_TURNS` | `4` | 短期记忆保留的最近工具轮数 |
| `IMG_SIZE` | `768` | 截图短边尺寸 |
| `GROUNDING_WIDTH/HEIGHT` | `1000` | 视觉定位输入分辨率 |
| `LOOP_DETECT_TODOLIST_STALE_ROUNDS` | `20` | TodoList 停滞多少轮触发反思 |
...
| `WEB_SEARCH_ENGINE` | `"tavily"` | 搜索引擎选择（tavily / firecrawl） |
...
每次 LLM 调用后读取 response.usage_metadata.total_tokens
   ├─ > 50% 窗口 → 轻量压缩：保留本轮首个 agent 动作
   ├─ > 75% 窗口 → 全量压缩：仅保留当前 user 消息
   └─ 就地替换 state["messages"] = [SystemMessage(摘要) + 保留部分]
```

压缩发生在 `call_model` 内部（图内每步），而非图外一次性处理，**子 Agent 自动受益**。

### 3. 循环检测两级响应

| 级别 | 触发条件 | 响应 |
|------|----------|------|
| ⚠️ 警告 | 连续 3 次同工具同参 / TodoList 停滞 20 轮 | 注入反思提示引导模型自我纠偏 |
...
| `audio.js` | 语音录制与播放 |
| `i18n.js` | 国际化 |
| `token-bar.js` | Token 用量条 |
| `pending.js` / `pending-overlay.js` | 待确认操作 |
| `rollback.js` | 消息回滚 |
| `workspace.js` | 工作空间切换 |
| `dialog.js` | 对话框 |
| `utils.js` | 通用工具 |
...
`variables.css` 定义主题变量，其余按模块拆分：`base / layout / chat / composer / cron / dialog / settings / skills / todo / toolcalls / edit-mode / audio / animations / pending / pending-overlay / responsive`。

### 添加自定义工具

1. 在 `src/agent/tools/` 新建 `my_tool.py`，继承 LangChain `BaseTool`，实现 `_run`，填写 `name` / `description` / `args_schema`
2. 在 `tool_config.json` 注册并配置启用状态与参数
3. （可选）在 `tool_policy.py` 设置是否需要人工确认
4. 重启后端，工具自动绑定到 LLM
...
- **工具调用轨迹**：`history/tool_calling/{session_id}/turn_{id}.json`
...
| 💾 **历史存储** | 会话历史与工具轨迹落盘在 `history/`，长期使用注意清理 |
